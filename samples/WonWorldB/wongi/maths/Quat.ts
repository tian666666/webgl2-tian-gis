...
class Quaternion extends Float32Array {
  isModified:boolean;
  constructor(q?: Quaternion | number[]) {
    super(4);
    if (
      q &&
      (q instanceof Quaternion || (q.length === 4 && Array.isArray(q)))
    ) {
      this[0] = q[0];
      this[1] = q[1];
      this[2] = q[2];
  ...
  rx(rad: number) {
    Quaternion.rotateX(this, this, rad);
    this.isModified = true;
    return this;
  }
  ry(rad: number) {
    Quaternion.rotateY(this, this, rad);
    this.isModified = true;
    return this;
  }
  rz(rad: number) {
    Quaternion.rotateZ(this, this, rad);
    this.isModified = true;
    return this;
  }

  setAxisAngle(axis: Vec3, angle: number) {
  ...
  static rotateVec3(qa: Quaternion, va: Vec3, out?: Vec3) {
    ...
    //			+ (s*s - dot(u, u)) * v
    //			+ 2.0f * s * cross(u, v);
    const q = new Vec3([qa[0], qa[1], qa[2]]), //Save the vector part of the Quaternion
      v = new Vec3([va[0], va[1], va[2]]), //Make a copy of the vector, going to chg its value
      s = qa[3], //Save Quaternion Scalar (W)
      d = Vec3.dot(new Vec3(q), new Vec3(v)), // U DOT V
      dq = Vec3.dot(new Vec3(q), new Vec3(q)), // U DOT U
      cqv = Vec3.cross(new Vec3(q), new Vec3(v), new Vec3([0, 0, 0])); // Cross Product for Q,V
  ...
  static lookRotation(
    vDir: number | Vec3 | number[],
    vUp: number | Vec3 | number[],
    out?: Quaternion
  ) {
  ...
  static toEuler(q: Quaternion, out?: Vec3) {
    //order="YZX"
    const x = q[0],
      y = q[1],
      z = q[2],
  ...
  static lerp(a: Quaternion, b: Quaternion, t: number, out?: Quaternion) {
    const ax = a[0],
      ay = a[1],
      az = a[2],
      aw = a[3];
  ...
  //https://github.com/toji/gl-matrix/blob/master/src/gl-matrix/quat.js
  static invert(a: Quaternion, out: Quaternion) {
    let a0 = a[0],
      a1 = a[1],
      a2 = a[2],
      a3 = a[3],
